...
# Metal Blockchain Node Operator - SIMPLE Deployment Script
# Just the core Metal node, no complex monitoring
...
echo -e "${BLUE}🏗️  Metal Blockchain Node - Simple Deployment${NC}"
echo "=============================================="
echo -e "${YELLOW}📝 This deploys just the core Metal node (no monitoring)${NC}"
echo ""
...
calculate_revenue() {
    ...
    local akash_costs=160  # Reduced costs without monitoring
    ...
    printf "   Akash Costs: ~$%.0f/month (simplified)\n" "$akash_costs"
    printf "   Net Profit: $%.0f - $%.0f/month\n" "$(echo "$monthly_low - $akash_costs" | bc -l)" "$(echo "$monthly_high - $akash_costs" | bc -l)"
}
...
echo -e "${BLUE}📦 What's included in this deployment:${NC}"
echo "   ✅ Metal blockchain node (MetalGo)"
echo "   ✅ Validator staking setup"
echo "   ✅ Nginx proxy for security"
echo "   ✅ Persistent storage for blockchain data"
echo "   ❌ No monitoring dashboards (separate template)"
echo ""

# Confirm deployment
read -p "🚀 Deploy simple Metal Node? (y/N): " confirm
if [[ ! $confirm =~ ^[Yy]$ ]]; then
...
if [ $BALANCE_AKT -lt 3 ]; then
    echo -e "${RED}❌ Insufficient balance: ${BALANCE_AKT} AKT${NC}"
    echo "You need at least 3 AKT to deploy simple Metal Node"
...
DEPLOY_FILE="deploy-simple-${VALIDATOR_NAME}.yml"
cp deploy-simple.yml "$DEPLOY_FILE"
...
# Deploy to Akash
echo -e "${BLUE}🚀 Deploying simple Metal Node to Akash...${NC}"
echo "This will create a deployment and escrow ~3 AKT..."
...
if [ $? -eq 0 ]; then
    ...
    echo ""
    echo -e "${GREEN}🎉 Your Metal validator will start earning staking rewards!${NC}"
    echo ""
    echo -e "${YELLOW}📊 To add monitoring later:${NC}"
    echo "   Run the monitoring add-on template after your node is stable"
    echo "   This keeps things simple and debuggable"
    
    # Save deployment info
    echo "VALIDATOR_NAME=$VALIDATOR_NAME" > "deployment-simple-${VALIDATOR_NAME}.env"
    echo "METAL_WALLET=$METAL_WALLET" >> "deployment-simple-${VALIDATOR_NAME}.env"
    echo "STAKING_AMOUNT=$STAKING_AMOUNT" >> "deployment-simple-${VALIDATOR_NAME}.env"
    echo "DEPLOY_FILE=$DEPLOY_FILE" >> "deployment-simple-${VALIDATOR_NAME}.env"
    
    echo -e "${BLUE}💾 Deployment info saved to: deployment-simple-${VALIDATOR_NAME}.env${NC}"
else
